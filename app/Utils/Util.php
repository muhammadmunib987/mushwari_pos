...
class Util
{
    ...
    private function sendCustomSmsWhatsapp($data)
    {
        $sms_settings = $data['sms_settings'];

    if (!empty($sms_settings['enable_gezya']) && $sms_settings['enable_gezya'] == 1 && !empty($sms_settings['gezya_sms'])) {
            $gezyaData = [
                'mobile_number' => $data['mobile_number'],
                'sms_body' => $data['sms_body'],
                'schedule' => $data['schedule'] ?? null,
                'device' => $sms_settings['gezya_device'] ?? 0,
                'is_mms' => $data['is_mms'] ?? false,
                'attachments' => $data['attachments'] ?? null,
                'prioritize' => $data['prioritize'] ?? false,
                'sms_settings' => $sms_settings
            ];
            
            $this->sendSmsViaGezya($gezyaData);
        }

  if (!empty($sms_settings['enable_whatsapp']) && $sms_settings['enable_whatsapp'] == 1 &&
        !empty($sms_settings['whatsapp_api_key']) && !empty($sms_settings['whatsapp_account_id'])) {
        $this->sendWhatsappViaMushwari($data);
    }
        return true;
    }
    ...
    private function sendSmsViaGezya($data)
    {
        $number = $data['mobile_number'] ?? null;
        $message = $data['sms_body'] ?? null;
        $sms_settings = $data['sms_settings'];
        ...
        $url = "https://sms.gezya.com/services/send.php";

        $postData = [
            'number' => $number,
            'message' => $message,
            'schedule' => $data['schedule'] ?? null,
            'key' => $sms_settings['gezya_sms'] ?? null,
            'devices' => $data['device'] ?? 0,
            'type' => !empty($data['is_mms']) ? 'mms' : 'sms',
            'attachments' => $data['attachments'] ?? null,
            'prioritize' => !empty($data['prioritize']) ? 1 : 0,
        ];
        ...
        if ($httpCode == 200) {
            ...
            if ($json && isset($json['success']) && $json['success']) {
                return $json['data']['messages'][0] ?? true;
            } else {
        ...
        private function sendWhatsappViaMushwari($data)
    {
        $sms_settings = $data['sms_settings'];

        if (empty($sms_settings['whatsapp_api_key']) || empty($sms_settings['whatsapp_account_id'])) {
            return false;
        }

        $url = "https://business.mushwari.com/api/send/whatsapp";

        $numbers = explode(',', trim($data['mobile_number'])); 

        foreach ($numbers as $number) {
            $response = Http::asForm()->post($url, [
                'secret' => $sms_settings['whatsapp_api_key'],
                'account' => $sms_settings['whatsapp_account_id'],
                'recipient' => $number,
                'type' => 'text',
            ...
            if (!$response->successful()) {
                Log::error('WhatsApp sending failed for number: ' . $number, [
    ...
    public function sendSms($data)
    {
        ...
        if ($sms_service == 'custom_sms_whatsapp') {
            return $this->sendCustomSmsWhatsapp($data);
        }
