...
class ProductUtil extends Util
{
    ...
    function getStockUnitWise($product, $stock) {
            $res = $product['sub_unit_ids'];
            $subUnits = json_decode($res, true); // decode subunit IDs
            if (!is_array($subUnits) || empty($subUnits)) {
                $unit = Unit::find($product['unit_id']);
                $unitName = $unit ? $unit->actual_name : 'Unit';
                ...
                // Fetch units from DB and sort by base_unit_multiplier DESC
                $units = Unit::whereIn('id', $subUnits)
                            ->orderByDesc('base_unit_multiplier')
                            ->get(['id', 'actual_name', 'base_unit_multiplier']);
            
                $result = [];
                $remaining = $stock;
            
                foreach ($units as $unit) {
                    if ($unit->base_unit_multiplier == 0) continue; // avoid division by zero
            
                    $qty = intdiv($remaining, $unit->base_unit_multiplier); // integer division
                    if ($qty > 0) {
                        $result[] = "$qty {$unit->actual_name}";
                        $remaining = $remaining % $unit->base_unit_multiplier;
                    }
                }
            
                // If there's any leftover and no unit handled it
                if ($remaining > 0) {
                    $smallestUnit = $units->sortBy('base_unit_multiplier')->first();
                    $result[] = "$remaining {$smallestUnit->actual_name}";
                }
