...
class LabelsController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $templates = DB::table('label_templates')
              ->get();
              
        return view('labels.show')
            ->with(compact('products','templates', 'barcode_settings', 'default', 'price_groups'));
    }
    ...
    public function preview(Request $request)
    {
        try {
            ...
            foreach ($products as $value) {
                ...
                $details->category_name = Category::where('id', $details->category_id)->value('name') ?? '';
...
// 2️⃣ Fetch the template from the database (use query builder safely)
$template = DB::table('label_templates')
              ->where('id', $templateId)
              ->first();

if (!$template || empty($template->body)) {
    abort(404, 'Label template not found or body is empty.');
}

$output = view("labels.partials.$template->view_name", [
    'print'           => $print,
