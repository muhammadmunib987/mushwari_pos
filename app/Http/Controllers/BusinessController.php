    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name','enable_fbr_taxpayer', 'pos_id', 'pos_token', 'start_date', 'currency_id', 'time_zone',
                'fy_start_month', 'accounting_method', 'tax_label_1','enable_fbr_taxpayer', 'pos_id', 'pos_token', 'tax_number_1',
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id','enable_fbr_taxpayer', 'pos_id', 'pos_token', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
    ...
    public function updateFbrTaxpayerStatus(Request $request) {
        ...
        try {
            ...
            $business->enable_fbr_taxpayer = !$business->enable_fbr_taxpayer;
            $business->save();

            $output = ['success' => 1,
                        'msg' => __("FBR Tax Payer API Status changed successfully!")
                    ];
        } catch (\Exception $e) {
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
