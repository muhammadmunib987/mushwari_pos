...
Route::get('/test-gezya-sms', function () {
    try {
        $url = "https://sms.gezya.com/services/send.php";

        $postData = [
            'number' => '00923154192110', // Replace with a test number
            'message' => 'Hello from Laravel to Gezya!',
            'schedule' => null, // or date string if needed
            'key' => '2e06b2579e90b9bf4112a8fc3255f31c01ce17a5',
            'devices' => 0, // or device ID if needed
            'type' => 'sms', // or 'mms'
            'attachments' => null, // path/url if mms
            'prioritize' => 0
        ];
        ...
        if ($response->successful()) {
            ...
            if (isset($json['success']) && $json['success']) {
                return "✅ Message Sent: " . json_encode($json['data']['messages'][0]);
            } else {
...
Route::get('/test-whatsapp', function () {
    ...
    $response = Http::asForm()->post($url, [
        'secret' => 'e22f039a5759faaafcb20189d338db2f234ba4ec',
        'account' => '1748520712c4ca4238a0b923820dcc509a6f75849b68384f0827ac1',
        'recipient' => '00923154192110', // Replace with the actual recipient number
        'type' => 'text',
        'message' => 'Hello from Laravel HTTP client!',
    ]);

    if ($response->successful()) {
        return "Success: " . $response->body();
    } else {
        return "Error: HTTP Code " . $response->status() . ", Response: " . $response->body();
    }
});
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::post('/business/update-taxpayer-status', [BusinessController::class, 'updateFbrTaxpayerStatus'])->name('business.update_fbr_taxpayer_status');
    ...
    Route::get('createSubunit', [UnitController::class,'createSubunit']);
    Route::post('subunitstore', [UnitController::class,'subunitstore']);
