...
class SellController extends ApiController
{
    ...
    $perPage = !empty($filters['per_page']) ? (int) $filters['per_page'] : $this->perPage;

if ($perPage == -1) {
    // Get all data without pagination
    $sells = $query->orderby('id','DESC')->get();

    return response()->json([
        'success' => $sells->isNotEmpty(),
        'message' => $sells->isNotEmpty() ? 'Get data successful' : 'No data found',
    ...
    $data = SellResource::collection(collect($paginator->items()));
