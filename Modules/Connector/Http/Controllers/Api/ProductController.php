    ...
    public function index()
    {
      ...
      $products = $this->__getProducts($business_id, $filters, $search, true, $order_by, $order_direction);
        ...
        if (
            ...
            $data = ProductResource::collection(collect($products->items()));
            return $this->paginatedResponse($products, $data);
        } else {
            return response()->json([
                'success' => $products->isNotEmpty(),
                'message' => $products->isNotEmpty() ? 'Get data successful' : 'No data found',
...
public function getproduct(Request $request)
{
    ...
    // ---- Inputs ------------------------------------------------------------
    $perPage     = $request->integer('per_page', 10);          // -1 = “all”
    $location_id = $request->integer('location_id');           // null if absent
    $searchTerm  = $request->filled('name')
        ? trim($request->input('name'))
        : null;

    // Business locations for the response payload
    $customer_businessLocations = DB::table('business_locations')
        ->select('id', 'business_id', 'name')
        ->where('business_id', $business_id)
        ->get();

    // ---- Query -------------------------------------------------------------
    $query = Product::select('id', 'name', 'business_id', 'image')
        ->where('business_id', $business_id)

        // filter by location_id (both in the main query and relation)
        ->when($location_id, function ($q) use ($location_id) {
            $q->whereHas('product_locations', function ($sub) use ($location_id) {
                $sub->where('product_locations.location_id', $location_id);
            });
        })
        ...
        // eager-load only the matching locations (saves memory)
    ...
    // ---- Fetch -------------------------------------------------------------
    $products = $perPage != -1 ? $query->paginate($perPage) : $query->get();

    // normalize to a Collection for mapping
    $collection = $products instanceof \Illuminate\Pagination\AbstractPaginator
        ? $products->getCollection()
        : $products;

    $data = $collection->map(fn ($product) => [
        'id'          => $product->id,
        'name'        => $product->name,
        'business_id' => $product->business_id,
        'price'       => '62.00',   // dummy price
        'discount'    => '50.00',   // dummy discount
        'image_url'   => $product->image
            ? asset('uploads/products/' . $product->image)
            : 'https://business.jadidkhata.com/img/default.png',
    ]);
    ...
    if ($products instanceof \Illuminate\Pagination\AbstractPaginator) {
        return response()->json([
            ...
            'data'             => $data,
            'businessLocations'=> $customer_businessLocations,
            'pagination'       => [
    ...
    return response()->json([
        ...
        'message' => $data->isNotEmpty() ? 'Get data successful' : 'No data found',
        'data'    => $data,
    ]);
}
    ...
    {
        $query = Product::select('id','name','business_id')->where('business_id', $business_id);
