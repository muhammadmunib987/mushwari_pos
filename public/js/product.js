...
$(document).on('submit', 'form#quick_add_subunit_form', function(e) {
    e.preventDefault();
    var unit_id = $('#unit_id').val();
    $('#subunit_base_unit_id').val(unit_id);

    var unitSku = $('#unit_sku').val();
    
    var productSkuField = $('#product_sku');
    var currentProductSkus = productSkuField.val();

    // Append the new SKU to the list, separated by commas
    if (currentProductSkus.length > 0) {
        currentProductSkus += ', ' + unitSku;
    } else {
        currentProductSkus = unitSku;
    }

    // Update the Product SKU field
    productSkuField.val(currentProductSkus);
